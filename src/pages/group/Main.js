...
function Main() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await fetchGroups({
          category: activeLink === '전체' ? '전체' : activeLink,
          currentPage: page,
    ...
    fetchData();
  }, [activeLink, region, subRegion, isRecruiting, page]);
  ...
  <Grid container spacing={2} columns={12}>
    ...
    {cards.map((cardItem, index) => (
      <Grid item xs={4} sm={4} md={4} key={cardItem.group.groupId || `card-${index}`}>
        <CardItem
          to={`/group/detail/${cardItem.group.groupId}`}
          category={cardItem.group.categoryId}
          location={`${cardItem.group.city}`}
          title={cardItem.group.groupName}
          date={cardItem.dday === 0 ? '오늘' : cardItem.dday > 0 ? `D-${cardItem.dday}` : `D+${Math.abs(cardItem.dday)}`}
          ...
          imageUrl={cardItem.group.groupImg}
        />
      </Grid>
    ))}
